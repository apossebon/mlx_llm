...
class RenderHarmony:
    ...
    def __init__(self):
        ...
        # Regex para detectar tool calls
        self._pattern_tool_call = (
            r'commentary\s+to=functions\.([^<\s]+)'
            r'(?:\s+<\|constrain\|>(\w+))?<\|message\|>(.*?)<\|call\|>'
        )
        self._pattern_analysis = r'<\|channel\|>analysis<\|message\|>(.*?)(?=<\|\w+\|>|<\|end\|>|\Z)'
        self._pattern_final_message = r'<\|channel\|>final<\|message\|>(.*?)(?=<\|\w+\|>|<\|end\|>|\Z)'
        self._pattern_commentary = r'<\|channel\|>commentary(?!\s+to=functions\.)(?:<\|constrain\|>(\w+))?<\|message\|>(.*?)(?=<\|\w+\|>|<\|end\|>|\Z)'
    ...
    def detect_harmony_tool_calls(self, response: str) -> List[Dict]:
        ...
        for m in re.finditer(self._pattern_tool_call, response, re.DOTALL):
            fn = m.group(1)
    ...
    def detect_harmony_analysis_messages(self, response: str) -> List[str]:
        ...
        for m in re.finditer(self._pattern_analysis, response, re.DOTALL):
    ...
    def detect_harmony_final_messages(self, response: str) -> List[str]:
        ...
        for m in re.finditer(self._pattern_final_message, response, re.DOTALL):
            if m == "":
                continue
            msgs.append(m.group(1).strip())
        return msgs
    ...
    def detect_harmony_commentary_messages(self, response: str) -> List[str]:
        ...
        for m in re.finditer(self._pattern_commentary, response, re.DOTALL):
            msgs.append(m.group(1).strip())
        return msgs
